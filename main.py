        ...
        db.add_all(
            [
                ...
                User(
                    name="Manutenção",
                    username="manutencao",
                    password_hash=pwd_context.hash("manutencao123"),
                    role="manutencao",
                ),
                User(
...
for upload in files:
    ...
    safe_name = (
        f"{req.id}_{secrets.token_hex(8)}{ext}"
    )
...
rows = (
    ...
    .filter(
        MaintenanceRequest.requester_id == user.id
    )
    .order_by(
...
counts = {
    "pendente": (
        db.query(MaintenanceRequest)
        .filter(
            MaintenanceRequest.status == "PENDENTE"
        )
        .count()
    ),
    "atendimento": (
...
add_history(
    ...
    req,
    user,
    f"Prioridade alterada de {old} para {priority}",
)
